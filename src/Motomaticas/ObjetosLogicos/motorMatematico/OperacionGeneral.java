...
public class OperacionGeneral implements Serializable {
	...
	private List<ObjetoMatematico> SubdividirOperacion(List<ObjetoMatematico> ListaOriginal,ObjetoMatematico PrimerObjeto, ObjetoMatematico UltimoObjeto) {

		List<ObjetoMatematico> SubLista = new CopyOnWriteArrayList<ObjetoMatematico>((ListaOriginal.subList(ListaOriginal.indexOf(PrimerObjeto) + 1, ListaOriginal.indexOf(UltimoObjeto))));
	...
	private TipoDeErrorMatematico ResolverSegundoOrden(List<ObjetoMatematico> ListaParResolverSegundoOrden,
		...
		for (int Index = 0; Index < ListaParResolverSegundoOrden.size(); Index++) {
			if (ListaParResolverSegundoOrden.get(Index).getTipoDeObjetoMatematico() == TipoObjetoMatematico.Operacion && ((OperacionMatematica)ListaParResolverSegundoOrden.get(Index)).getPrioridadDeOperacion() == prioridad ) {
				try {
	...
	private TipoDeErrorMatematico ResolverOrdenN(List<ObjetoMatematico> ListaDeOrdenPrevia,List<ObjetoMatematico> ListaDeOrdenN[],int prioridad) {

		List<ObjetoMatematico>[] ListaArraySegundoOrden = new List[1];//puesto como array para poder usarlo como un apuntador y
		...
		TipoDeErrorMatematico posibleError = ResolverSegundoOrden(ListaDeOrdenPrevia,ListaArraySegundoOrden,prioridad);	
	...
	private TipoDeErrorMatematico ResolverTercerOrden() {
		...
		for (ObjetoMatematico ObjetoActual : ObjetosMatematicos_TercerOrden) {
			if (ObjetoUltimo != null) {
				if (ObjetoActual.getTipoDeObjetoMatematico() == TipoObjetoMatematico.Unidad) { /// IDENTIFICAR EL TIPO DE OBJETO ACTUAL
	...
	public TipoDeErrorMatematico CalcularOperacion() {
		...
		TipoDeErrorMatematico CuartoOrden = ResolverTercerOrden();
		if (CuartoOrden != null) {
			Error = CuartoOrden;
			return CuartoOrden;
		}
